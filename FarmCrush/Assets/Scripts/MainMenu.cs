...
public class MainMenu : MonoBehaviour {
	...
	void OnGUI() {
		...
		GUI.Button (new Rect(loginButtonRect.x,loginButtonRect.y+50,loginButtonRect.width,loginButtonRect.height), "xx", "loginButton");
	}
	
	private IEnumerator ParseLogin() {
		if (FB.IsLoggedIn) {
			// Logging in with Parse
			System.Threading.Tasks.Task<ParseUser> loginTask = ParseFacebookUtils.LogInAsync(FB.UserId, 
			                                              FB.AccessToken, 
